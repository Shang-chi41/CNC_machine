...
export class CNCKinematicsMultiGroup {
  ...
  _buildVirtualLinks() {
    ...
    for (const [linkName, linkCfg] of Object.entries(this.map.links)) {
      const group = this.linkGroups.get(linkName);
      const requiredObjects = linkCfg.objects || [];
      const optionalObjects = linkCfg.optional_objects || [];
      for (const objectName of [...requiredObjects, ...optionalObjects]) {
        const obj = this._findObject(objectName);
        if (!obj) {
          const log = optionalObjects.includes(objectName) ? console.info : console.warn;
          log(`[CNCKinematics] ${optionalObjects.includes(objectName) ? 'Optional' : 'Missing'} GLB object: ${objectName}`);
          continue;
        }
