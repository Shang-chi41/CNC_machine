...
window.setMode = function (mode) {
    ...
    if (mode === 'auto') { if (window.setAiCollapsed) window.setAiCollapsed(false); fetchGcodeList(); }
    if (mode === 'manual' && window.setAiCollapsed) window.setAiCollapsed(true);
};
...
async function fetchMachineStatus() {
    try {
        const d     = await api.get('/api/control/status').catch(() => api.get('/api/pose/latest'));
...
function _renderGcodeList(data) {
    ...
    const executingRow = el.querySelector('.gc-item.is-executing');
    if (executingRow) executingRow.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
}
...
function pollStreamProgress(totalLines) {
    ...
    _streamPollInterval = setInterval(async () => {
        try {
            ...
            const status = await api.get('/api/control/status').catch(() => api.get('/api/pose/latest'));
