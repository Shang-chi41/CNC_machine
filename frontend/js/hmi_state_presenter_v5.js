...
function operatorBlockedReasons(s) {
  ...
  if (upper(s.sync) !== 'MACHINE_NX_SYNCED') reasons.push('Chưa cài đặt gốc tọa độ và đồng bộ máy với mô phỏng.');
...
function applyOperatorState(payload) {
  ...
  const primaryReason = $('opPrimaryReason');
  if (primaryReason) {
    const text = ready ? 'Đủ điều kiện chính. Chỉ RUN khi phôi, dao và vùng làm việc đã sẵn sàng.' : (reasons[0] || action);
    primaryReason.textContent = text;
    primaryReason.classList.toggle('ready', ready);
  }
...
function connectWs() {
  ...
  const token = localStorage.getItem('cnc_token') || '';
    ...
    const url = `${proto}//${window.location.host}/ws/pose${qs}`;
  let timer;
  const go = () => {
...
function initSidebarStatusTimeout() {
  setTimeout(() => {
    const edgeText = $('sEdgeTxt');
    const edgeDot = $('sEdgeDot');
    if (edgeText && /edge\.\.\.|loading|---/i.test(edgeText.textContent || '')) {
      edgeText.textContent = 'Không kết nối';
      if (edgeDot) edgeDot.className = 'sdot off disconnected';
    }
    const almText = $('sAlmTxt');
    const almDot = $('sAlmDot');
    if (almText && /alarm\.\.\.|loading|---/i.test(almText.textContent || '')) {
      almText.textContent = 'Chưa có dữ liệu';
      if (almDot) almDot.className = 'sdot idle';
    }
  }, 5000);
}
